...
\begin{proof}{Теоремы~\ref{th:bidir_corr}}
  ...
  \begin{itemize}
    \item открывающая скобка $(_k$
      ...
      \textit{$p \colon (q_1, u) \rightarrow (q_{4/7}, v) \rightarrow (q_1, w) \rightarrow \dots \rightarrow (q_f, z)$.}

      Рёбра с меткой $(_k$ соответствуют рёбрам входного графа, а именно рёбрам $u \xrightarrow{(_k)} v$ и $w \xrightarrow{(_k)} v$. Заметим, что тогда (по двунаправленности графа) существует путь $(_k )_k$ из $u$ в $w$, а значит существует $S$-ребро $\q{q_1, u} \to \q{q_2, w}$. Также, по предположению индукции существует простой путь $\q{q_1, w} \to \q{q_f, z}$, а значит есть и $S$-ребро $\q{q_2, w} \to \q{q_3, z}$. Вместе эти два ребра формируют просто путь $\q{q_1, u} \to \q{q_2, w} \to \q{q_3, z}$, что и хотелось.

      % Так как рёбра с меткой $(_{1/2}$ выходят только из начального слоя
